...
import { BookOpen, PieChart, ArrowLeft, Download, ChevronDown, Plus, GitBranch, Settings, X, Loader2, Radio } from 'lucide-react';
import { downloadPdf } from './utils/pdfExport';
import { ProjectProvider, useProject } from './context/ProjectContext';

// Main Content Wrapper to consume Context
const AppContent: React.FC = () => {
    const { state, startInvestigation, createBranch, resetProject, setActiveBranch, updateActiveBook, loadDemoData } = useProject();
            ...
            <>
                ...
                <div className="text-center mt-4">
                    <button
                        ...
                        className="px-6 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-lg text-sm font-bold"
                    >
            ...
            <header className="h-16 border-b border-gray-800 bg-black/50 backdrop-blur-md sticky top-0 z-50 flex items-center justify-between px-4 lg:px-6">
                ...
                <div className="flex bg-gray-900 rounded-lg p-1 border border-gray-800">
                    ...
                    <button
                        ...
                        className={`px-3 md:px-4 py-1.5 rounded-md text-sm font-medium flex items-center gap-2 transition-all ${activeTab === 'PODCAST' ? 'bg-pink-500 text-white shadow-sm' : 'text-gray-500 hover:text-gray-300'}`}
                    >
                        <Radio size={14} /> <span className="hidden md:inline">Podcast</span>
                    </button>
                </div>
                ...
                {state.project && (
                    <>
                        {activeTab === 'RESEARCH' ? (
                            ...
                            activeBranch ? (
                                <PodcastStudio book={activeBranch.book} />
                            ) : (
                                <div className="text-center text-gray-500 py-20">Generate a book first to create a podcast.</div>
                            )
                        ) : (
